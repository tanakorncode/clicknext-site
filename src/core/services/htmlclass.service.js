import { ADD_BODY_CLASSNAME, ADD_CLASSNAME, REMOVE_BODY_CLASSNAME } from "@/store/htmlclass.module";
...
const HtmlClass = {
  ...
  initHeader() {
    ...
    if (objectPath.get(this.config, "header.menu.self.display")) {
      store.dispatch(ADD_CLASSNAME, {
        ...
        className: `header-menu-layout-${objectPath.get(this.config, "header.menu.self.layout")}`,
      });
  ...
  initSubheader() {
    ...
    if (objectPath.has(this.config, "subheader.style")) {
      store.dispatch(ADD_BODY_CLASSNAME, `subheader-${objectPath.get(this.config, "subheader.style")}`);
    }
  },
